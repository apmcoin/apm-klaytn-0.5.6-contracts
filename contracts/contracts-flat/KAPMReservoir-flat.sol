...
contract KAPMReservoir is IAPMReservoir {
    ...
    function _takeAmount(
        ...
        if (paysFee && feeRecipient != address(0)) {
            ...
            if (feeAmount != 0) {
                IKApmCoin(token).transferFrom(msg.sender, feeRecipient, feeAmount);
                emit TransferFee(msg.sender, feeRecipient, feeAmount);
            }
        }
        IKApmCoin(token).burnFrom(msg.sender, amount);
    }
    ...
    function _giveAmount(
        ...
        if (!isFeePayed && protocolFee != 0 && senderDiscountRate != 10000) {
            ...
            if (feeRecipient != address(0) && receiverDiscountRate != 10000) {
                ...
                if (feeAmount != 0) {
                    IKApmCoin(token).mint(feeRecipient, feeAmount);
                    emit TransferFee(receiver, feeRecipient, feeAmount);
                }
            }
        }
        IKApmCoin(token).mint(receiver, amount - feeAmount);
    }
    ...
    function migrate(
        ...
        if (klayBalance > 0) {
            (bool success, ) = payable(newReservoir).call{value: klayBalance}("");
            require(success, "klay transfer failure");
        }

        uint256 KApmBalance = IKApmCoin(token).balanceOf(address(this));
        if (KApmBalance > 0) IKApmCoin(token).transfer(newReservoir, KApmBalance);

        IKApmCoin(token).addMinter(newReservoir);
        IKApmCoin(token).renounceMinter();
